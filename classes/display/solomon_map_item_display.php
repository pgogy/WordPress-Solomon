<?PHP
	...
	class solomon_map_item_display{
		...
		function scripts(){
			...
			if(isset($post->post_type)){
				...
				if($post->post_type!="solomonmap"&&$post->post_type!="solomonmapwalk"){
					if(strpos($post->post_type,"solomonmap")===0){
							...
							.ready(
								function(){	
										...
										if($post->post_type=="solomonmaptlabel"){ 
											...
											$default = plugin_basename(__FILE__);
											$parts = explode("/", $default); 
											$default = site_url() . $parts[0] . "/leaflet/images/marker-icon.png";
										
										?>
											display_item_text_label("<?PHP echo $post->ID; ?>", "<?PHP echo $default; ?>"); 
										<?PHP } 	
									
										if($post->post_type=="solomonmaplabel"){ ?>
											display_item_label("<?PHP echo $post->ID; ?>"); 
										<?PHP } 	

										if($post->post_type=="solomonmapioverlay"){ ?>
											display_ioverlay("<?PHP echo $post->ID; ?>"); 
										<?PHP } 

										if($post->post_type=="solomonmapimage"){ ?>
											display_image("<?PHP echo $post->ID; ?>"); 
										<?PHP } ?>
								}
							);
	...
	$solomon_map_item_display = new solomon_map_item_display();
